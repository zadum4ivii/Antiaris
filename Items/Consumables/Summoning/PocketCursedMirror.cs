...
namespace Antiaris.Items.Consumables.Summoning
{
    public class PocketCursedMirror : ModItem
    {
		...
		{
			return Main.hardMode && player.ZoneCorrupt && !NPC.AnyNPCs(mod.NPCType("TowerKeeperNonActive")) && !NPC.AnyNPCs(mod.NPCType("TowerKeeper")) && !NPC.AnyNPCs(mod.NPCType("TowerKeeper2"));
		}
        ...
        public override void AddRecipes()
        {
			...
			recipe.AddIngredient(null, "ShadowChargedCrystal", 5);
        ...
        public override bool UseItem(Player player)
        {
            ...
            if (Main.netMode != 1)
            {
                NPC.SpawnOnPlayer(player.whoAmI, mod.NPCType("TowerKeeper2"));
            }
            return true;
        }
    }
