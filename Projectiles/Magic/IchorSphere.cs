...
namespace Antiaris.Projectiles.Magic
{
    public class IchorSphere : ModProjectile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            DisplayName.AddTranslation(GameCulture.Russian, "Сфера ихора");
			DisplayName.AddTranslation(GameCulture.Chinese, "脓血球");
        ...
        public override void Kill(int timeLeft)
        {
            for (int k = 0; k < 7; k++)
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 159, (float)Main.rand.Next(-4, 4), (float)Main.rand.Next(-4, 4), 0, new Color(), 1.5f);
                Main.dust[dust].noGravity = true;
            }
        }
    }
