...
namespace Antiaris.Projectiles.Magic
{
    public class Torrent : ModProjectile
    {
        ...
        {
            this.SetTag("extinguisher");
        }

        public override void AI()
        {
			if (projectile.timeLeft <= 60) projectile.velocity.Y += 0.075f;
			for (int k = 0; k < 3; k++)
            {
		...
		{ 
			for (int k = -1; k <= 1; k += 2) Projectile.NewProjectile(projectile.Center, Vector2.Zero - new Vector2(-0.16f * -(float)k, 2.64f), mod.ProjectileType("Copy" + GetType().Name), (int)(projectile.damage * 0.8f), projectile.knockBack, projectile.owner);
