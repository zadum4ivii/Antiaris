...
namespace Antiaris.Projectiles.Magic
{
    public class YinYangBlackM : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int k = 0; k < 2; k++)
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType("YinYangBlack"), projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.2f, 100, new Color(), 1.6f);
        ...
        public override void Kill(int timeLeft)
        {
            for (int k = 0; k < 15; k++)
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height,  mod.DustType("YinYangBlack"), (float)Main.rand.Next(-4, 4), (float)Main.rand.Next(-4, 4), 0, new Color(), 1.5f);
                Main.dust[dust].noGravity = true;
            }
        }
    }
