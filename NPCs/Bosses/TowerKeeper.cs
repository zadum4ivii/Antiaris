        ...
        public override void AI()
        {
            ...
            if (!this.secondState)
            {
                ...
                for (int count = 0; count < 10; count++)
                {
                    ...
                    NPC.NewNPC((int)spawn.X, (int)spawn.Y, mod.NPCType("ProtectiveStone"), 0, (float)npc.whoAmI, 0.0f, (float)count, 0.0f, 255);
                }
            ...
            if ((double)npc.life <= npc.lifeMax * 0.333)
            {
                ...
                if ((double)npc.ai[1] % 200.0 == 0 && (double)npc.ai[1] <= 399.0)
                {
                    ...
                    if (this.attackTimer <= 2)
                    {
                        ...
                        for (int k = 0; k < (Main.expertMode ? 5 : 3); k++) Projectile.NewProjectile(shootPos.X + (float)(-100 * npc.direction) + (float)Main.rand.Next(-40, 41), shootPos.Y - (float)Main.rand.Next(-50, 40), shootVel.X, shootVel.Y, mod.ProjectileType("GolemCrystal"), npc.damage / 3, 5f);
                    }
                    else
                    {
                        if (Main.expertMode)
                        {
                            for (int i = 0; i < 7; i++)
                                Projectile.NewProjectile((int)((player.position.X - 50) + Main.rand.Next(100)), (int)((player.position.Y - 50) + Main.rand.Next(100)), 0.0f, 0.0f, mod.ProjectileType("TowerKeeper1Sheet"), npc.damage / 3, 4.5f);
                        }
                        for (int k = 0; k < (Main.expertMode ? 8 : 5); k++)
                        {
                            ...
                            Projectile.NewProjectile(shootPos, shootVel, mod.ProjectileType("GolemCrystal"), npc.damage / 3, 4.5f);
                        }
                        Main.PlaySound(2, (int)npc.position.X, (int)npc.position.Y, 88);
                        this.attackTimer = 0;
                    }
			...
			if ((double)npc.life <= npc.lifeMax * 0.135)
            {
                ...
                if (NPC.AnyNPCs(mod.NPCType("ProtectiveStone")))
                {
                    //if ((double)this.ai % 215.0 == 0.0)
					if(Main.rand.Next(5) == 0)
                        Projectile.NewProjectile((int)((npc.position.X - 500) + Main.rand.Next(1000)), (int)((npc.position.Y - 500) + Main.rand.Next(1000)), 0.0f, 0.0f, mod.ProjectileType("TowerKeeper1Sheet"), npc.damage / 3, 4.5f);
