    ...
    public class PetrousKnight1 : ModNPC
    {
        ...
        public override void NPCLoot()
        {
            if (Main.netMode != 1)
            {
                ...
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("StoneHammer1"), 1, false, 0, false, false);
                }
            }
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				for (int k = 0; k < 50; k++)
				{
					Dust.NewDust(npc.position, npc.width, npc.height, 62, 2.5f * (float)hitDirection, -2.5f, 0, default(Color), 1.2f);
				}
