...
namespace Antiaris.NPCs.Enemies
{
    public class PurplePixie : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            bannerItem = mod.ItemType("PurplePixieBanner");
            banner = npc.type;
        }
        ...
        public override void SetStaticDefaults()
        {
            ...
            DisplayName.AddTranslation(GameCulture.Russian, "Фиолетовая пикси");
            DisplayName.AddTranslation(GameCulture.Chinese, "紫色小精灵");
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(npc.position, npc.width, npc.height, 173, 0.0f, 0.0f, 200, default(Color), 1.0f);
            ...
            if (this.spawning < 3)
                ...
                {
                    this.spawning++;
                    NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("PurplePetitePixie"), 0, 0.0f, (float)npc.whoAmI, 0.0f, 0.0f, 255);
                }
                ...
                if (!moon)
                {
                    ...
                    else
                    {
                        ...
                        foreach (NPC npc1 in Main.npc)
                            if (npc1.active && npc1.type == mod.NPCType("PurplePetitePixie"))
                            {
        ...
        {
            if (Main.rand.Next(2) == 0)
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("PurpleBigCrystal"), Main.rand.Next(1, 3), false, 0, false, false);
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            {
                for (int k = 0; k < 20; k++)
                    Dust.NewDust(npc.position, npc.width, npc.height, 173, 2.5f * (float)hitDirection, -2.5f, 0, default(Color), 0.7f);
            }
            else
            {
                for (int k = 0; k < 50; k++)
